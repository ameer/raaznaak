<template>
  <div>
    <v-item-group v-model="selectedZone">
      <v-container>
        <v-row>
          <v-col v-for="(zone, i) in zones" :key="`zc-${i}`" cols="6">
  ...
  computed: {
    ...
    placesInfo () {
      if (this.selectedZone !== null) {
        return this.zones[this.selectedZone].places
      }
      return []
    }
  },
